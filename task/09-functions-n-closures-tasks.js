...
function getPolynom() {
    ...
    return function (x) {
        return args.reduce((accum, curr, index) => {
                return accum + curr * Math.pow(x, index);
    });
    }
}
...
function logger(func, logFunc) {
    return function () {
        ...
        let funcCall = JSON.stringify(args);
        funcCall = funcCall.slice(1, -1);
        funcCall = `${func.name}(${funcCall})`;
